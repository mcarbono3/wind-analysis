...
def calculate_metrics(df, caribbean_bbox, analysis_point_lat, analysis_point_lon, radius_km=200):
    ...
    df["latitude_float"] = df["latitude"].str[:-1].astype(float) * (1 if "N" in df["latitude"].str[-1] else -1)
    df["longitude_float"] = df["longitude"].str[:-1].astype(float) * (-1 if "W" in df["longitude"].str[-1] else 1)
    ...
    nearby_events["year"] = pd.to_datetime(nearby_events["date"], format="%Y%m%d").dt.year
...
df = pd.read_csv("parsed_hurdat_data.csv")
